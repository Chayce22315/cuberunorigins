...
final class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        // Camera will be configured after player is created

        // Initialise state machine with MainMenuState as entry point
        let mainMenu = MainMenuState(stateMachine: nil)
        stateMachine = GameStateMachine(initialState: MainMenuState.self)
        // Link the state back to the state machine
        mainMenu.stateMachineWrapper = stateMachine
    }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if player == nil, let gsm = stateMachine, gsm.currentState is GameplayState {
